...
console.log("\n===== 6. 실제 월드에서 =====");
{
  ...
  // 도심이 높아지면 한쪽에만 건물이 있는 지점이 늘어난다. 표본을 넣어 개수를 확보한다.
  const spots = [[0, 0], [180, 60], [-140, 220], [90, -190], [-260, -80],
                 [320, 140], [-320, -220], [60, 340], [-60, -340], [240, -260], [-200, 120]];
  ...
  for (const [sx, sz] of spots) {
    ...
    // 도심 한복판처럼 한 건물 벽면이 압도적으로 좋은 자리에서는 A 든 D 든 같은 앵커가
    // 최선이라 선택이 안 갈린다 (초고층으로 바꾼 뒤 실제로 그런 지점이 생겼다).
    // 이 시험이 보는 건 "고를 게 둘 있을 때 이동 입력이 결정하는가" 다. 선택지가
    // 하나뿐인 자리는 전제가 성립하지 않으므로 세지 않는다.
    if (Math.abs(l.x - r.x) < 1) { skipped++; continue; }
    tried++;
  ...
  ok(win === tried, "좌우에 다른 선택지가 있으면 A 는 항상 왼쪽에 건다", `${win}/${tried} · ${sample}`);
